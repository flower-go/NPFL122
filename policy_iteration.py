...
def computeActions(state):
    res = []
    
    for a in range(len(GridWorld.actions)):
        rew = 0
        step = GridWorld.step(state,a)
        for p in range(len(step)):
                rew += step[p][0]*(step[p][1] + args.gamma * value_function[step[p][2]])
        res.append(rew)
...
if __name__ == "__main__":
    ...
    for stepCount in range(args.steps):
        for iteration in range(args.iterations):
        #POLICY EVALUATION
            value_F2 = [0] * GridWorld.states
            for state in range(GridWorld.states):     
                step = GridWorld.step(state,policy[state])               
                for p in range(len(step)):
                    value_F2[state] += step[p][0]*(step[p][1] + args.gamma * value_function[step[p][2]])
            value_function = value_F2[:]
                
        #POLICY IMPROVEMENT
        for state in range(GridWorld.states):
                actionIndex = computeActions(state)
                policy[state] = np.argmax(actionIndex)
